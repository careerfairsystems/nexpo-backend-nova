...
# Define the column indices for G to O (6 to 14 if using zero-based indexing)
time_columns_indices = list(range(6, 15))  # G=6, H=7, ..., O=14

ticket_list = []

for index, row in df.iterrows():
    if 5 <= index < 195:  # Adjust the row numbers for the emails and tickets as needed
        email = row[3]  # Assuming the email is in the 4th column
        total_tickets = row[4]  # Assuming the total tickets are in the 5th column
...
# Output to a file
output_file_path = '/Users/victormikkelsen/ARKAD/2023/IT/nexpo-backend-nova/UploadToDB/lunch_tickets.json'
