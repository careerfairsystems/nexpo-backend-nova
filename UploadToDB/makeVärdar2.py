...
# Load the Excel file
df = pd.read_excel('/Users/victormikkelsen/ARKAD/2023/IT/nexpo-backend-nova/UploadToDB/värdar.xlsx', sheet_name='Värdar Biljetter', header=None)

# Define the column indices for F to I (5 to 8 if using zero-based indexing)
time_columns_indices = list(range(9, 13))  # F=5, G=6, H=7, I=8

ticket_list = []

for index, row in df.iterrows():
    if 5 <= index < 272:  # Adjust the row numbers for the emails and tickets as needed
        ...
        ticket_json = {
            "tickets": {
                "mail": email,
                "eventid": 60,  # Assuming a new event ID for the updated script
...
# Output to a file
output_file_path = '/Users/victormikkelsen/ARKAD/2023/IT/nexpo-backend-nova/UploadToDB/host_tickets2.json'
